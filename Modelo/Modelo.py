...
from guardado_datos import guardar_variables, registro_tiempos, registro_medicos, registro_costos
...
# Variable que indica el numero de personal libre de cada especialidad
...
# Registro de tiempos
lista_tiempos = []
t_total = (check_restr_f - check_p_in) / 60
lista_tiempos.append(["Total (min)", t_total])
...
# Horas en operación de cada médico
print()
lista_medicos = []
for m in M:
    sum_y = 0
    ...
    # Horas extra
    horas_extra = quicksum(
        (1 - JOR[m, d, t]) * y[p, m, d, t].x for p in P for d in D for t in T)

    lista_medicos.append([m, int(sum_y), horas_extra.getValue()])

registro_medicos(lista_medicos)


# Impresión de costos (F.O.)
CMedExtra = quicksum(
    (1 - JOR[m, d, t]) * y[p, m, d, t].x * CM[m] * PON[m] for p in P for m in M for d in D for t in T)

CMed = quicksum(JOR[m, d, t] * y[p, m, d, t].x * CM[m]
                for p in P for m in M for d in D for t in T)

CPers = quicksum(y[p, m, d, t].x * REQ[p, a] * CA[a]
                 for p in P for m in M for a in A for d in D for t in T)

CIns = quicksum(x[p, m, d, t].x * CI[p]
                for p in P for m in M for d in D for t in T)

lista_costos = []
lista_costos.append(["Costo médicos", CMed.getValue()])
lista_costos.append(["Costo médicos extra", CMedExtra.getValue()])
lista_costos.append(["Costo personal médico", CPers.getValue()])
lista_costos.append(["Costo insumos", CIns.getValue()])

registro_costos(lista_costos)

print(f"Costo médicos: {CMed.getValue()}")
print(f"Costo médicos extra: {CMedExtra.getValue()}")
print(f"Costo personal médico: {CPers.getValue()}")
print(f"Costo insumos: {CIns.getValue()}")
