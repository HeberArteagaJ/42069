...
model.addConstr(quicksum(quicksum(quicksum(
    quicksum(x[p, m, d, t] for m in M) for d in D) for t in T) for p in P) >= len(P) * POR)
...
lista_tiempos.append(["Total", t_total])
...
print(f"Costo médicos: {CMed}")
print(f"Costo médicos extra: {CMedExtra}")
print(f"Costo personal médico: {CPers}")
print(f"Costo insumos: {CIns}")
...
for m in M:
    ...
    print(f"Médico {m}: {horas_extra} bloques extra")
